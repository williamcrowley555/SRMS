...
if(!empty($_POST["classid1"])) 
{
 ...
 $status=0;	
 $stmt = $dbh->prepare("SELECT tblsubjects.SubjectName,tblsubjects.id FROM tblsubjectcombination join  tblsubjects on  tblsubjects.id=tblsubjectcombination.SubjectId WHERE tblsubjectcombination.FacultyId=(SELECT FacultyId FROM tblclasses WHERE id=:cid) and tblsubjectcombination.status!=:stts order by tblsubjects.SubjectName");
